...
A a = new A()와 같이 생성자를 호출해 객체를 만들어 왔지만 ,   
 __지금까지 생성자를 만든 기억은 없다__ 생성자를 만들지 않아도 정상적으로 호출 된 이유는   
 클래스에게 컴파일러가 기본 생성자를 추가해 줬기 때문이다.   
 기본 생성자 : 입력매개변수 X   
...
<br>
<br>
클래스 외부에서 멤버(필드, 메섣, 이너 클래스)를 호출하기 위해서는 객체를 먼저 생성한다.   
참조변수명.멤버명 형태로 호출하지만, 클래스 내부에서는 객체의 생성 없이 필드와 메서드를 바로 사용할 수 있습니다.   
...
우리는 클래스 내부에서도 객체 안의 멤버를 사용해 왔다.   
참조 변수명.멤버명 형태를 사용해 온거다 사실.   
다만 객체를 직접 만들지 않은 것뿐이다.   

모든 메서드에는 자신이 포함된 클래스의 객체를 가리키는 __this__ 라는 참조 변수 있다.
우리가 int m = 3 필드를 클래스 내부에서 출력하고자 할 때 System.out.println(this.m)와 같이 작성해야 된다.   
    
this. 생략하면 컴파일러가 자동으로 this. 추가해 주기 떄문에 지금까지 클래스 내부에서 필드와 메서드를 그대로 사용할 수 있었던 것이다.   

<br>

> this()

자신이 속한 클래스 내부의 다른 생성장를 호출하는 명령어   
